...
function buildAutoPayload(
  ...
  const derivativePanelTemplates = [
    {
      timeframe: "4H" as const,
      bias: pickByDate(`${input.analysisDate}-4h-bias`, ["bearish", "neutral", "bullish"] as const),
      oi: {
        value: localizedSignal?.oiValue || pickByDate(`${input.analysisDate}-4h-oi-value`, ["$8.12B", "$8.34B", "$8.56B"]),
        stance: localizedSignal?.oiState || pickByDate(`${input.analysisDate}-4h-oi-stance`, ["短线增仓回流", "持仓变化有限", "去杠杆延续"]),
      },
      funding: {
        value: localizedSignal?.fundingValue || pickByDate(`${input.analysisDate}-4h-funding-value`, ["0.008%", "0.011%", "-0.004%"]),
        stance: localizedSignal?.fundingState || pickByDate(`${input.analysisDate}-4h-funding-stance`, ["轻微偏多", "相对中性", "轻微偏空"]),
      },
      cvd: {
        value: localizedSignal?.cvdValue || pickByDate(`${input.analysisDate}-4h-cvd-value`, ["+182M", "+96M", "-74M"]),
        stance: localizedSignal?.cvdState || pickByDate(`${input.analysisDate}-4h-cvd-stance`, ["主动买盘占优", "主动流平衡", "主动卖盘占优"]),
      },
    },
    {
      ...
      oi: {
        value: localizedSignal?.oiValue || pickByDate(`${input.analysisDate}-d-oi-value`, ["$8.42B", "$8.68B", "$8.91B"]),
        stance: localizedSignal?.oiState || pickByDate(`${input.analysisDate}-d-oi-stance`, ["整体增仓扩张", "结构仍在整理", "持仓回落释放"]),
      },
      funding: {
        value: localizedSignal?.fundingValue || pickByDate(`${input.analysisDate}-d-funding-value`, ["0.012%", "0.016%", "-0.006%"]),
        stance: localizedSignal?.fundingState || pickByDate(`${input.analysisDate}-d-funding-stance`, ["多头拥挤抬升", "资金费率平衡", "空头拥挤抬升"]),
      },
      cvd: {
        value: localizedSignal?.cvdValue || pickByDate(`${input.analysisDate}-d-cvd-value`, ["+420M", "+210M", "-185M"]),
        stance: localizedSignal?.cvdState || pickByDate(`${input.analysisDate}-d-cvd-stance`, ["日内主动买盘延续", "主动流尚未明显倾斜", "日内主动卖压占优"]),
      },
    },
    {
      ...
      oi: {
        value: localizedSignal?.oiValue || pickByDate(`${input.analysisDate}-w-oi-value`, ["$8.95B", "$9.18B", "$9.46B"]),
        stance: localizedSignal?.oiState || pickByDate(`${input.analysisDate}-w-oi-stance`, ["周级别持仓仍高", "高位持仓震荡", "周级别持仓释放"]),
      },
      funding: {
        value: localizedSignal?.fundingValue || pickByDate(`${input.analysisDate}-w-funding-value`, ["0.021%", "0.013%", "-0.009%"]),
        stance: localizedSignal?.fundingState || pickByDate(`${input.analysisDate}-w-funding-stance`, ["周级别多头略拥挤", "周级别平衡", "周级别空头回补"]),
      },
      cvd: {
        value: localizedSignal?.cvdValue || pickByDate(`${input.analysisDate}-w-cvd-value`, ["+1.24B", "+620M", "-540M"]),
        stance: localizedSignal?.cvdState || pickByDate(`${input.analysisDate}-w-cvd-stance`, ["更大级别买盘仍在", "周级别主动流分歧", "更大级别卖盘仍占优"]),
      },
    },
  ];
  const indicatorPanels = derivativePanelTemplates.map((panel) => ({
    timeframe: panel.timeframe,
    bias: panel.bias,
    vwap: {
      value: panel.oi.value,
      val: panel.funding.value,
      vah: localizedSignal?.liquidationValue || pickByDate(`${input.analysisDate}-${panel.timeframe}-liq-value`, ["$98M", "$126M", "$154M"]),
      stance: panel.oi.stance,
    },
    macd: {
      direction: panel.funding.stance,
      divergence: localizedSignal?.liquidationState || pickByDate(`${input.analysisDate}-${panel.timeframe}-liq-stance`, ["多空双向清算", "空头主导爆仓", "多头主导爆仓"]),
      bias: panel.bias,
    },
    rsi: {
      value: Number.parseInt(String(panel.cvd.value).replace(/[^0-9-]/g, "").slice(0, 3) || "0", 10),
      divergence: panel.cvd.stance,
      bias: panel.bias,
    },
  }));
  ...
  const payload: DailyAiMarketAnalysis = {
    ...
    vwap: localizedSignal?.oiState || "优先观察未平仓合约变化",
    macd: localizedSignal?.fundingState || "优先观察资金费率是否继续偏移",
    rsi: localizedSignal?.cvdState || "优先观察主动买卖流是否延续",
    indicatorPanels,
